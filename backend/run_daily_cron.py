#!/usr/bin/env python3
"""
Скрипт для запуска ежедневных задач через Railway Cron.
...
ENDPOINT = f"{BACKEND_URL}/scheduler/run-daily-tasks"

async def main():
    """Выполняет HTTP запрос к эндпоинту ежедневных задач."""
    print(f"--- Запуск ежедневных задач через Railway Cron ---")
